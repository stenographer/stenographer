...
/// Objects that conform to the Endpoint protocol may be used by an `SXLogger` instance as Log Entry destinations.
public protocol SXEndpoint {

    /// The minimum Priority Level a Log Entry must meet to be accepted by this Endpoint.
    var minimumPriorityLevel: SXPriorityLevel { get }
    /// The formatter used by this Endpoint to serialize a Log Entry’s `dateTime` property to a string.
    var dateFormatter: SXDateFormatter { get }
    /// The formatter used by this Endpoint to serialize each Log Entry to a string.
    var entryFormatter: SXEntryFormatter { get }
