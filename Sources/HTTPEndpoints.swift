 ...
 */
private class SXPersistedCache {
     ...
     *
     *  - Paramters:
     *      - timeoutInterval: The amount of time data will remain reserved before assuming the
     *                         upload failed, and allowing the data to be tried again.
     *      - fileName: The cache file's name. This file will be created in the directory indicated
     *                  by `SX_DEFAULT_LOG_DIRECTORY`.
     */
    fileprivate init(timeoutInterval: TimeInterval, fileName: String) {
        self.timeoutInterval = timeoutInterval
        if let fileURL = SX_DEFAULT_LOG_DIRECTORY?.appendingPathComponent(fileName, isDirectory: false) {
    ...
    /**
     *  Call this method when data has failed to upload. The cache will end the data's reservation
     *  and allow a subsequent attempt to upload the data again.
     */
 ...
 */
open class SXHTTPEndpoint: SXEndpoint {
    ...
    fileprivate lazy var cache: SXPersistedCache = SXPersistedCache(timeoutInterval: 50, fileName: self.cacheName)
     ...
     *
     *  - Paramters:
     *      - request: The request that will be used when submitting uploads.
     *      - successCodes: The set of HTTP status codes the server might respond with to indicate a
     *                      successful upload. Defaults to `{200, 201, 202, 204}`.
     *      - sessionConfiguration: The configuration to be used when initializating this Endpoint's
     *                              URL session. Defaults to `.defaultSessionConfiguration()`.
     *      - minimumPriorityLevel: The minimum Priority Level a Log Entry must meet to be accepted
     *                              by this Endpoint. Defaults to `.All`.
     ...
     *      - entryFormatter: The formatter used by this Endpoint to serialize each Log Entry to a
     ...
     *
     *  - Paramters:
     *      - URL: The URL to upload Log Entries to.
    ...
    @objc fileprivate func upload(_ timer: Timer?) {
        if #available(OSXApplicationExtension 10.10, *) {
            SX_STENOGRAPHER_QUEUE.async(execute: {
 ...
 *  but makes no guarantees.
 */
open class SXHTTPJSONEndpoint: SXHTTPEndpoint {
    ...
    public init(
        ...
        sessionConfiguration: URLSessionConfiguration = URLSessionConfiguration.default,
        minimumPriorityLevel: SXPriorityLevel = .all,
        dateFormatter: SXDateFormatter = SXDateFormatter.ISO8601DateTimeFormatter()
    ) {
        super.init(
            request: request,
            successCodes: successCodes,
            ...
            minimumPriorityLevel: minimumPriorityLevel,
            dateFormatter: dateFormatter,
            entryFormatter: SXEntryFormatter.jsonFormatter()
        )
    }
