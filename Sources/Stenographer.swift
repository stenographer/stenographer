...
/// The version of the Stenographer framework currently in use.
internal let SX_STENOGRAPHER_VERSION = Bundle(identifier: "info.logkit.Stenographer")?.infoDictionary?["CFBundleShortVersionString"] as? String ?? "2.3.2"

/// The default queue used throughout the framework for background tasks.
internal let SX_STENOGRAPHER_QUEUE: DispatchQueue = DispatchQueue.global(qos: DispatchQoS.QoSClass.default)
...
/// The bundle ID of the currently running host application, _not_ the Stenographer framework.
internal let SX_BUNDLE_ID: String = Bundle.main.bundleIdentifier ?? ""

/// The model of this device.
internal let SX_DEVICE_MODEL: String = {
    var len: size_t = 0
...
internal let SX_DEVICE_TYPE: String = {
#if os(OSX)
    return "Mac"
...
internal let SX_DEVICE_OS: (description: String, majorVersion: Int, minorVersion: Int, patchVersion: Int, buildVersion: String) = {
 ...
 *
 *  - Important: Stenographer honors the iOS/tvOS `ASIdentifierManager.advertisingTrackingEnabled` flag. If an end user has
 *             disabled advertising tracking on their device, Stenographer will substitute an empty string for the
 *             `advertising` ID.
 *  - Note: The iOS/tvOS advertising ID is disabled by default to prevent triggering [IDFA requirements][idfa] in apps
 *        that do not require an advertising ID. To enable the `advertising` ID, the `-DStenographerAdvertisingIDDisabled`
 *        compiler flag must be removed from the Stenographer Project build settings. The flag is found in the "Swift
 *        Compiler - Custom Flags" section of the Build Settings page, under "Other Swift Flags". Be sure to search in
 *        the *Stenographer Project* build settings, not your app's project settings. Additionally, be sure to add/remove
 *        the flag from the Stenographer *Project* global build settings, not one of Stenographer's OS-specific targets.
 *
 ...
 */
internal let SX_DEVICE_IDS: (vendor: String, advertising: String) = {
#if os(OSX)
    ...
    #if StenographerAdvertisingIDDisabled
        let advertisingID = ""
    #else
