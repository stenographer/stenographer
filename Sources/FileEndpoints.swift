 ...
 *  contains the current and next URLs, at the `SXFileEndpointRotationCurrentURLKey` and
 *  `SXFileEndpointRotationNextURLKey` keys, respectively.
 *
 *  This notification is send _before_ the rotation occurs.
 */
public let SXFileEndpointWillRotateFilesNotification: String = "info.logkit.endpoint.fileEndpoint.willRotateFiles"
 ...
 *
 *  The notification's `object` is the actual Endpoint instance that is rotating files. The `userInfo` dictionary
 *  contains the current and previous URLs, at the `SXFileEndpointRotationCurrentURLKey` and
 *  `SXFileEndpointRotationPreviousURLKey` keys, respectively.
 *
 *  This notification is send _after_ the rotation occurs, but _before_ any pending Log Entries have been written to
 *  the new file.
 */
public let SXFileEndpointDidRotateFilesNotification:  String = "info.logkit.endpoint.fileEndpoint.didRotateFiles"

/// The value found at this key is the `NSURL` of the sender's previous log file.
public let SXFileEndpointRotationPreviousURLKey:      String = "info.logkit.endpoint.fileEndpoint.previousURL"

/// The value found at this key is the `NSURL` of the sender's current log file.
public let SXFileEndpointRotationCurrentURLKey:       String = "info.logkit.endpoint.fileEndpoint.currentURL"

/// The value found at this key is the `NSURL` of the sender's next log file.
public let SXFileEndpointRotationNextURLKey:          String = "info.logkit.endpoint.fileEndpoint.nextURL"

/// The default file to use when logging: `log.txt`
public let defaultLogFileURL: URL? = SX_DEFAULT_LOG_DIRECTORY?.appendingPathComponent("log.txt", isDirectory: false)
...
private class SXLogFile {
     ...
     *
     *      - shouldAppend: Indicates whether new data should be appended to existing data in the file, or if
 ...
 *
 *  The notifications `SXFileEndpointWillRotateFilesNotification` and `SXFileEndpointDidRotateFilesNotification`
 *  are sent to the default notification center directly before and after rotating log files.
 */
open class RotatingFileEndpoint: SXEndpoint {
    ...
    fileprivate lazy var currentFile: SXLogFile? = { [unowned self] in
        guard let file = try? SXLogFile(URL: self.currentURL, shouldAppend: true) else {
            assertionFailure("Could not open the log file at URL '\(self.currentURL.absoluteString)'")
            return nil
        }
        file.setExtendedAttribute(name: self.extendedAttributeKey, value: SX_STENOGRAPHER_VERSION)
        return file
    }()
     ...
     *
     *  - Parameters:
     *      - baseURL: The URL used to build the rotating file set’s file URLs. Each file's index
     ...
     *      - numberOfFiles: The number of files to be used in the rotation. Defaults to `5`.
     *      - maxFileSizeKiB: The maximum file size of each file in the rotation, specified in kilobytes.
    ...
    fileprivate func nextFile() -> SXLogFile? {
        guard let nextFile = try? SXLogFile(URL: self.nextURL, shouldAppend: false) else {
            assertionFailure("The log file at URL '\(self.nextURL)' could not be opened.")
            return nil
        }
        nextFile.setExtendedAttribute(name: self.extendedAttributeKey, value: SX_STENOGRAPHER_VERSION)
        return nextFile
    }
    ...
    fileprivate func rotateToFile(_ nextFile: SXLogFile) {
        ...
        NotificationCenter.default.post(
            name: Notification.Name(rawValue: SXFileEndpointWillRotateFilesNotification),
            object: self,
            userInfo: [
                SXFileEndpointRotationCurrentURLKey: self.currentURL,
                SXFileEndpointRotationNextURLKey: self.nextURL
            ]
        )
        ...
        NotificationCenter.default.post(
            name: Notification.Name(rawValue: SXFileEndpointDidRotateFilesNotification),
            object: self,
            userInfo: [
                SXFileEndpointRotationCurrentURLKey: self.currentURL,
                SXFileEndpointRotationPreviousURLKey: previousURL
            ]
        )
    }
...
open class FileEndpoint: RotatingFileEndpoint {
     ...
     *      - shouldAppend: Indicates whether the Endpoint should continue appending Log Entries to the
 ...
 *
 *  The notifications `SXFileEndpointWillRotateFilesNotification` and `SXFileEndpointDidRotateFilesNotification` are
...
open class DatedFileEndpoint: RotatingFileEndpoint {
    ...
    fileprivate let nameFormatter = SXDateFormatter.dateOnlyFormatter()
     ...
     *
     *  - Paramters:
     *      - baseURL: The URL used to build the date files’ URLs. Today's date will be prepended
...
// MARK: Aliases
// ======================================================================== //
// Classes in LogKit 3.0 will drop the SX prefixes. To facilitate other 3.0
 ...
 */
public typealias SXRotatingFileEndpoint = RotatingFileEndpoint

/**
 *  An Endpoint that writes Log Entries to a specified file.
 *  - note: This is a LogKit 3.0 forward-compatibility typealias to `FileEndpoint`.
 */
public typealias SXFileEndpoint = FileEndpoint
 ...
 */
public typealias SXDatedFileEndpoint = DatedFileEndpoint
