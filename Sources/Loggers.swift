...
public final class SXLogger {

    /// The collection of Endpoints that successfully initialized.
    fileprivate let endpoints: [SXEndpoint]
     ...
     */
    public init(endpoints: [SXEndpoint?]) {
    ...
    public convenience init() {
        self.init(endpoints: [SXConsoleEndpoint()])
    }
    ...
    fileprivate func log(
        ...
        if !targetEndpoints.isEmpty {
            ...
            for endpoint in targetEndpoints {
                let entryString = endpoint.entryFormatter.stringFromEntry(SXLogEntry(
                    message: message,
                    userInfo: userInfo,
     ...
     *      - userInfo: A dictionary of additional values for Endpoints to consider.
     */
    public func debug(
